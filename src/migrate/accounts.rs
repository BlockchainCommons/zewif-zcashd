...
use zewif::{
    Account, ProtocolAddress, TxId, sapling::SaplingExtendedSpendingKey,
};
...
use crate::{
    Result, ZcashdWallet,
    zcashd_wallet::{
        Address, UfvkFingerprint, UnifiedAccounts, WalletTx,
...
pub fn convert_unified_accounts(
    ...
    for (key_id, account_metadata) in &unified_accounts.account_metadata {
        ...
        let account_name =
            format!("Account #{}", account_metadata.zip32_account_id());
    ...
    let address_registry =
        initialize_address_registry(wallet, unified_accounts)?;
    ...
    for (sapling_address, viewing_key) in wallet.sapling_z_addresses() {
            ...
            .and_then(|account_key_id| accounts_map.get_mut(account_key_id))
        {
            ...
            if let Some(sapling_key) =
                find_sapling_key_for_ivk(wallet, viewing_key)
            {
                // Convert to Zewif spending key format
                shielded_address.set_spending_key(
                    SaplingExtendedSpendingKey::new(
            ...
            if let Some(purpose) =
                wallet.address_purposes().get(&zcashd_address)
            {
                zewif_address.set_purpose(purpose.clone());
            }
    ...
    //         // Use the registry to find all addresses associated with this
    // account         let account_addresses =
    // address_registry.find_addresses_for_account(key_id);         if
    // !account_addresses.is_empty() {             eprintln!("  Account has
    // {} addresses", account_addresses.len());         }
    //     }
    // }

    // Step 5: Assign transactions to relevant accounts based on address
    // involvement We'll use our AddressRegistry to find account
    // associations
    ...
    for (txid, wallet_tx) in wallet.transactions() {
        ...
        match extract_transaction_addresses(wallet, *txid, wallet_tx) {
            Ok(tx_addresses) => {
                ...
                let is_change_transaction =
                    tx_addresses.contains("transaction_type:change");
                let transaction_type = if tx_addresses
                    .contains("transaction_type:send")
                {
                    "send"
                ...
                // First pass: Look for explicit account mappings from standard
                // addresses
                for address_str in &tx_addresses {
                    // Check for standard addresses that we can convert to
                    // AddressId
                    if let Ok(addr_id) =
                        AddressId::from_address_string(address_str)
                    {
                        // Look up the account in our registry
                        if let Some(account_id) =
                            address_registry.find_account(&addr_id)
                        {
                ...
                // Second pass: Check for tagged addresses with better
                // identifiers
                if relevant_accounts.is_empty() {
                    for address_str in &tx_addresses {
                        ...
                        {
                            // This is a spending address - may indicate source
                            // account
                            let pure_addr = &address_str
                                [(address_str.find(':').unwrap() + 1)..];
                            if let Ok(addr_id) =
                                AddressId::from_address_string(pure_addr)
                                && let Some(account_id) =
                                    address_registry.find_account(&addr_id)
                            {
                                relevant_accounts.insert(*account_id);
                        ...
                        {
                            // This is a change address - try to find its
                            // account
                            let pure_addr = &address_str
                                [(address_str.find(':').unwrap() + 1)..];
                            if let Ok(addr_id) =
                ...
                // If we still don't have accounts, use intelligent fallback
                // strategy
                if relevant_accounts.is_empty() {
                    // Different strategies based on transaction type
                    if is_change_transaction {
                        // For change transactions, try to find the source
                        // account
                        if let Some(source_account) =
                            find_source_account_for_transaction(
                                wallet_tx,
                                &tx_addresses,
                                &address_registry,
                            )
                        ...
                        // For send transactions with no clear mappings, look
                        // for the source
                        if let Some(source_account) =
                            find_source_account_for_transaction(
                                wallet_tx,
                        ...
                        // For receives, we could try to find the most likely
                        // recipient account Or fallback
                        // to the default account
                        if let Some(default_account) =
                            find_default_account_id(&accounts_map)
                        {
                ...
                // Last resort: If we still couldn't determine relevant
                // accounts, select a single appropriate account
                // rather than adding to all
                if relevant_accounts.is_empty() {
                    if let Some(default_account) =
                        find_default_account_id(&accounts_map)
                    {
            ...
            Err(e) => {
                ...
                // Even in error cases, try to assign to the default account if
                // possible
                if let Some(default_account_id) =
                    find_default_account_id(&accounts_map)
                {
                    if let Some(account) =
                        accounts_map.get_mut(&default_account_id)
                    {
                        account.add_relevant_transaction(*txid);
                    }
                } else {
...
/// Find the source account for a transaction based on transaction data and
/// extracted addresses
fn find_source_account_for_transaction(
    ...
    if wallet_tx.is_from_me() {
        for address_str in addresses {
            ...
            {
                let pure_addr =
                    &address_str[(address_str.find(':').unwrap() + 1)..];
            ...
            // Next, check for change addresses (these are most reliable for
            // source account)
...
/// Find the account ID for a transparent address by looking at key metadata and
/// relationships
fn find_account_for_transparent_address(
    ...
    // This requires looking up the pub key for this address and finding
    // matching key metadata

    // 1. Look up HD paths and seed fingerprints for all keys
    for key in wallet.keys().keypairs() {
        // We can't directly convert from pubkey to address, so iterate through
        // known addresses
        ...
        for known_addr in wallet.address_names().keys() {
            if known_addr.to_string() == addr_str {
                // Found the address in our address book. Now check if we can
                // link it to a key/account.
                ...
                if let Some(hd_path) = key.metadata().hd_keypath() {
                    ...
                    // - type' is 0 for external addresses, 1 for internal
                    //   (change) addresses
                    // - idx' is the address index

                    // If this is a unified account HD path, we can extract the
                    // account ID
                    if is_unified_account_keypath(hd_path)
                        && let Some(account_id) =
                            extract_account_id_from_keypath(hd_path)
                    {
                        // Look for unified account with this account ID
                        return find_account_key_id_by_account_id(
                            unified_accounts,
                            account_id,
                        );
                    }
                ...
                // If we have a key fingerprint, check if it matches any unified
                // account
                if let Some(seed_fp) = key.metadata().seed_fp() {
                    return find_account_key_id_by_seed_fingerprint(
                        unified_accounts,
                        seed_fp,
                    );
                }
...
/// Find the account ID for a sapling address by looking at the viewing key
/// relationships
fn find_account_for_sapling_address(
    ...
    // The unified-account plumbing for mapping Sapling addresses back to
    // account IDs has not been finalized yet. Rather than panicking we fall
    // back to `None` so callers can continue assigning the address to a
    // default bucket. This keeps migration usable while signalling that richer
...
fn extract_account_id_from_keypath(keypath: &str) -> Option<u32> {
    ...
    let parts: Vec<&str> = keypath.split('/').collect();
    if parts.len() >= 4
        && let Some(account_part) = parts.get(3)
        && let Some(account_str) = account_part.strip_suffix('\'')
    {
...
fn find_account_key_id_by_seed_fingerprint(
    ...
    for (key_id, account_metadata) in &unified_accounts.account_metadata {
        ...
        let account_seed_fp_hex =
            account_metadata.seed_fingerprint().to_hex().to_string();
...
pub(crate) fn initialize_address_registry(
    ...
    for address_metadata in &unified_accounts.address_metadata {
        ...
        let addr_id =
            AddressId::from_unified_address_metadata(address_metadata);
    ...
    for (sapling_address, viewing_key) in wallet.sapling_z_addresses() {
        ...
        if let Some(account_id) = find_account_for_sapling_address(
            wallet,
            unified_accounts,
            sapling_address,
            viewing_key,
