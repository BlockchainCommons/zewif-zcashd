...
pub fn extract_transaction_addresses(
    ...
    if let Some(recipients) = wallet.send_recipients().get(&tx_id) {
        for recipient in recipients {
            ...
            //            // If we find this address in our metadata, it's
            // likely a change address            if format!("{}",
            // addr_metadata.key_id) == recipient.unified_address {
            //                is_change_transaction = true;
            //                addresses.insert(format!("change:{}",
            // recipient.unified_address));                break;
            //            }
            //        }
            //    }
            //}
    ...
    if let Some(t_bundle) = tx.transaction().transparent_bundle() {
        for tx_in in t_bundle.vin.iter() {
            ...
            let input_addr =
                format!("input:{}:{}", txid_str, tx_in.prevout.n());
            ...
            if script_data.len() > 33 {
                ...
                if potential_pubkey[0] == 0x02 || potential_pubkey[0] == 0x03 {
                    ...
                    // Check if this is one of our keys to better determine
                    // ownership
                    for key in wallet.keys().keypairs() {
                        // Cannot directly convert PubKey to Address, so we'll
                        // check differently
                        // Get the address from our address book that might
                        // match this key
                        for (address, _) in wallet.address_names().iter() {
                            if address.to_string() == addr_str {
                                addresses
                                    .insert(format!("our_key:{}", addr_str));

                                // If we have an HD path, we can determine if
                                // this is change
                                if let Some(hd_path) =
                                    key.metadata().hd_keypath()
                                    && (hd_path.contains("/1'/")
                                        || hd_path.contains("/1/"))
                                {
                                    // This is an internal key path, so this is
                                    // likely change
                                    is_change_transaction = true;
                                    addresses.insert(format!(
                                        "change_key:{}",
        ...
        for (vout_idx, tx_out) in t_bundle.vout.iter().enumerate() {
                ...
                && script_data[1] == 0xA9
            {
                if script_data[23] == 0x88 && script_data[24] == 0xAC {
                    ...
                    let key_id = KeyId::from(
                        u160::from_slice(pubkey_hash)
                            .expect("Creating u160 from pubkey hash"),
                    );
                    let addr_str = key_id.to_string(wallet.network());
                    addresses.insert(addr_str.clone());
                    addresses
                        .insert(format!("transparent_output:{}", addr_str));
                ...
                && script_data[22] == 0x87
            {
                ...
                let script_id = ScriptId::from(
                    u160::from_slice(script_hash)
                        .expect("Creating u160 from script hash"),
                );
                let addr_str = script_id.to_string(wallet.network());
                addresses.insert(addr_str.clone());
                addresses
                    .insert(format!("transparent_script_output:{}", addr_str));
                output_address = addr_str;
            }

            // Check if this output is change
            if !output_address.is_empty() {
                // If this is our address and tx is from us, this is likely
                // change
                if tx.is_from_me()
                    && wallet
                        .address_names()
                ...
                {
                    // Check if this address isn't in our address book (typical
                    // of change addresses)
        ...
        for (outpoint, note_data) in sapling_note_data {
            ...
            for (addr, ivk) in wallet.sapling_z_addresses() {
                if note_data.incoming_viewing_key() == ivk {
                    ...
                    // Tag as input or output based on outpoint (outpoint is of
                    // type JSOutPoint)
                    ...
                    if note_data.nullifier().is_some() {
                        addresses
                            .insert(format!("sapling_spent_note:{}", addr_str));
                    } else {
                        addresses.insert(format!(
                            "sapling_unspent_note:{}",
                            addr_str
                        ));
                    }
                    break;
    ...
    // Orchard action processing is done after sapling, so we don't need to
    // process sapling note data again here
    ...
    if let Some(orchard_bundle) = tx.transaction().orchard_bundle() {
        for (idx, action) in (0u32..).zip(orchard_bundle.actions().into_iter())
        {
                ...
                && let Some(ivk) = orchard_meta.receiving_key(idx)
            {
                ...
                // Instead of trying to access note data directly, just track
                // the action Action data typically contains
                // commitment and value information
                addresses.insert(format!(
                    "orchard_action_data:{}",
                ...
                if let Some(recipients) = wallet.send_recipients().get(&tx_id) {
                    for recipient in recipients {
                        if let RecipientAddress::Orchard(addr) =
    ...
    // If the transaction is marked as "from me" but we don't have specific
    // addresses
    if tx.is_from_me()
    ...
    {
        // Add all our addresses as potential sources, but mark them as
        // uncertain
...
fn is_likely_change_output(wallet: &ZcashdWallet, address: &str) -> bool {
    ...
    // Check if the address belongs to our wallet but has no name or purpose in
    // the address book
    ...
    // If it's in our wallet but doesn't have a name or purpose, it's likely
    // change
    true
}
